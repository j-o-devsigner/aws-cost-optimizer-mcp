...
import type { EC2AnalysisResult, CPUMetrics } from '../types/index.js';

/**
 * Lista todas las instancias EC2 con su estado y detalles
 */
export async function listEC2Instances() {
  try {
    ...
    const instances: EC2AnalysisResult['instances'] = [];
    ...
    const result: EC2AnalysisResult = {
    ...
    return formatMCPError(error as Error);
  }
}

/**
 * Obtiene métricas de CPU de una instancia específica
 */
export async function getInstanceCPUUsage(instanceId: string) {
  try {
